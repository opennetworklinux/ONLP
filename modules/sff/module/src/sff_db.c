...
int
sff_db_get_type(sff_info_t* info, sff_module_type_t type)
{
    int i;
    sff_db_entry_t* entry;
    for(i = 0; i < AIM_ARRAYSIZE(sff_database__); i++) {
        entry = sff_database__ + i;
        if(entry->info.module_type == type) {
            memcpy(info, &entry->info, sizeof(*info));
            return 1;
        }
    }
    return 0;
...
int
sff_db_entry_struct(sff_info_t* info, aim_pvs_t* pvs)
{
    int i, j;
    aim_printf(pvs, "    {\n");
    aim_printf(pvs, "        {\n");
    ...
    for(i = 0; i < 16; i++) {
        aim_printf(pvs, "                ");
        for(j = 0; j < 16; j++) {
            aim_printf(pvs, "0x%.2x, ", info->eeprom[i*16+j]);
        }
        aim_printf(pvs, "\n");
    }
    aim_printf(pvs, "            },\n");
    ...
    aim_printf(pvs, "        SFF_%s_PROPERTIES,\n",
               sff_module_type_name(info->module_type));
    aim_printf(pvs, "        %d,\n", info->length);
    aim_printf(pvs, "        },\n");
    aim_printf(pvs, "    },\n");
    return 0;
