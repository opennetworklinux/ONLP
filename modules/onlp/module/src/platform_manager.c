    ...
    for(i = 0; i < AIM_ARRAYSIZE(psu_oid_table); i++) {
        ...
        if ( !flag[i] ) {
            ...
            if ( pi.status & ONLP_PSU_STATUS_FAILED ) {
                AIM_SYSLOG_WARN("PSU <id> is in failed state.",
                                "The given PSU is in failed state.",
                                "PSU %d is in failed state.", pid);
            }
    ...
    for(i = 0; i < AIM_ARRAYSIZE(fan_oid_table); i++) {
        ...
        if ( !flag[i] ) {
            ...
            if ( fi.status & ONLP_FAN_STATUS_FAILED ) {
                    AIM_SYSLOG_WARN("Fan <id> is in failed state.",
                                "The given fan is in failed state.",
                                "Fan %d is in failed state.", fid);
            }
           flag[i] = 1;
        }
